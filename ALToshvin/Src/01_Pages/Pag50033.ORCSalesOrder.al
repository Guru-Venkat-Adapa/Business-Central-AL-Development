page 50033 "ORC Sales Order"
{
    Caption = 'ORC Sales Order';
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("No."; Rec."No.")
                {
                    ...
                    var
                        ...
                        NoSeriesLine: Record "No. Series Relationship";
                        NoSeries: Record "No. Series";
                        SelectedSeriesCode: Code[20];
                        CustomNoSeriesCode: Code[20];
                        Window: Dialog;
                        Text051: Label 'The sales %1 %2 already exists.';
                    begin
                        SalesReceiveSetup.Get();
                        CustomNoSeriesCode := SalesReceiveSetup."ORC Order Nos. Series";
                        NoSeriesLine.SetRange(Code, CustomNoSeriesCode);
                        if NoSeriesLine.FindSet() then begin
                            if PAGE.RunModal(PAGE::"No. Series Relationships", NoSeriesLine) = ACTION::LookupOK then begin
                                SelectedSeriesCode := NoSeriesLine."Series Code";
                                if SelectedSeriesCode = '' then
                                    Error('No related number series selected.');
                                Rec."No. Series" := SelectedSeriesCode;
                                Rec."No." := NoSeriesMgt.GetNextNo(SelectedSeriesCode, Today(), true);
                            end
                            else
                                exit;
                        end;
                ...
                field("Purchase Order No Ref."; Rec."Purchase Order No Ref.")
                {
                    ApplicationArea = Suite;
                    Importance = Promoted;
                    ToolTip = 'Specifies the value of the Purchase Order No reference field.';
                }

                field("Sales Order No. Ref."; Rec."Sales Order No. Ref.")
                {
                    ApplicationArea = Suite;
                    Importance = Promoted;
                    ToolTip = 'Specifies the value of the Sales Order No reference field.';
                }

                field("Sale Invoice No. Ref."; Rec."Sale Invoice No. Ref.")
                {
                    ApplicationArea = Suite;
                    Importance = Promoted;
                    ToolTip = 'Specifies the value of the Sale Invoice No reference field.';
                }
                ...
                field("Sales Note Date"; Rec."Sales Note Date")
                {
                    ApplicationArea = All;
                    Caption = 'Sales Note Date';
                }
                ...
                field("Margin % (MS) "; Rec."Margin % (MS) ")
                {
                    ApplicationArea = All;
                    Caption = 'Margin % (MS)';
                }
                field("Margin Amount (MS)"; Rec."Margin Amount (MS)")
                {
                    ApplicationArea = All;
                    Caption = 'Margin Amount (MS)';
                }
            ...
            {
                Caption = 'ORC';
                //Visible = Rec."Sales Order Type" = 'Sales Order ORC';
                ...
                group(ExecutiveMaster)
                {
                    ...
                    field("Commission Note"; Rec."Commission Note")
                    {
                        ApplicationArea = All;
                        Caption = 'Commission Note';
                    }
                    field("Commission Note Date"; Rec."Commission Note Date")
                    {
                        ApplicationArea = All;
                        Caption = 'Commission Note Date';
                    }
                    field("CN Sent to SAP for Remit Dt"; Rec."CN Sent to SAP for Remit Dt")
                    {
                        ApplicationArea = All;
                        Caption = 'CN Sent to SAP for Remittance Date';
                    }
                    field("CN Remittance Received Date"; Rec."CN Remittance Received Date")
                    {
                        ApplicationArea = All;
                        Caption = 'CN Remittance Received Date';
                    }
                    field("CN Comments"; Rec."CN Comments")
                    {
                        ApplicationArea = All;
                        Caption = 'CN Comments';
                    }
            ...
            part(SalesLines; "ORC Sales Order Subform")
            {
                ApplicationArea = Basic, Suite;
            ...
            group("P&osting")
            {
                ...
                action(ProformaInvoice)
                {
                    ...
                    begin
                        ...
                        SalesHeader.SetFilter("Sales Order Type", '%1|%2', 'SPARES ORC', 'INSTRUMENT ORC');
                        if SalesHeader.FindFirst() then
                            Report.Run(Report::"ORC Sales Order Prof. Inv", true, false, SalesHeader)
                        else
                            Error('This document is not applicable for ORC Proforma Invoice. Sales Order Type must be SPARES ORC or INSTRUMENT ORC.');
                    end;
                    //TBC-1031 <-----
                }
                group("Prepa&yment")
