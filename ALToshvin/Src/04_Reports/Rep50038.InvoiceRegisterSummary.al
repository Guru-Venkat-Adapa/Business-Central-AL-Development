report 50038 "Invoice Register Summary"
{
    ApplicationArea = All;
    Caption = 'Invoice Register Summary';
    ...
    begin
        ...
        ExcelBuffer.AddColumn('Party PO Recd Date', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        ExcelBuffer.AddColumn('Vendor Bill No.', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        ExcelBuffer.AddColumn('Sales Order Date', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ExcelBuffer.AddColumn('Contact No.', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ExcelBuffer.AddColumn('Payment Term.Name', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ExcelBuffer.AddColumn('Payment-Term Description', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        //TBC-1006 --->
        ExcelBuffer.AddColumn('Invoice cancel status', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ExcelBuffer.AddColumn('Credit Memo Number.', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        ExcelBuffer.AddColumn('Invoice Amt. (Total Incl. VAT (INR)', false, '', true, false, false, '', ExcelBuffer."Cell Type"::Text);
        //TBC-1006 <-----
    ...
    begin
        ...
        if DepartmentName <> '' then
            ExcelBuffer.AddColumn(DepartmentName, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
        ...
        if Dimension.FindFirst() then
            RegionalOfficeName := Dimension.Name;

        if RegionalOfficeName <> '' then
            ExcelBuffer.AddColumn(RegionalOfficeName, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
        ...
        if Dimension.FindFirst() then
            RegionalGroupName := Dimension.Name;

        if RegionalGroupName <> '' then
            ExcelBuffer.AddColumn(RegionalGroupName, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
        ...
        //SalesAC.Name
        Clear(GLName);
        if GenPostingSetup.Get("Sales Invoice Line"."Gen. Bus. Posting Group", "Sales Invoice Line"."Gen. Prod. Posting Group")
        then begin
            GLEntry.Reset();
        ...
        if Cust.Get("Sales Invoice Header"."Sell-to Customer No.") then begin
            ...
            //CustomerAC.State Name & State Code
            if RecState.Get(Cust."State Code") then begin
                ExcelBuffer.AddColumn(RecState.Description, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn(RecState."State Code (GST Reg. No.)", false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
            end else begin
                ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
            end;
            ...
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text); // GSTIN
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text); // State Name
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text); // State Code
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text); // Key/Non Key
        end;
        ...
        if ILE.FindFirst() then begin
            ...
            if ItemAppEntry.FindSet() then
                ...
                until (ItemAppEntry.Next() = 0) or FoundReceipt;
        ...
        //GRN Posted Pur. Reci. No.
        if PurchReceiptNo <> '' then
            ExcelBuffer.AddColumn(PurchReceiptNo, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);

        //Party PO Recd Date
        if "Sales Invoice Header"."Party PO Received Date" <> 0D then
            ExcelBuffer.AddColumn("Sales Invoice Header"."Party PO Received Date", false, '', false, false, false, '', ExcelBuffer."Cell Type"::Date)
        else
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        //Margin Value
        Clear(MarginValue);
        MarginValue := TaxbleAmount - PurchaseValue;
        ...
        if "Sales Invoice Line"."Shipment No." <> '' then
            ExcelBuffer.AddColumn("Sales Invoice Line"."Shipment No.", false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        if "Sales Invoice Header"."Executive Master" <> '' then
            ExcelBuffer.AddColumn("Sales Invoice Header"."Executive Master", false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);

        //Executive Master-2.Name
        if "Sales Invoice Header"."Executive Master2" <> '' then
            ExcelBuffer.AddColumn("Sales Invoice Header"."Executive Master2", false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        if SalesCommentLine.FindSet() then
            repeat
                if Remarks = '' then
                    Remarks := SalesCommentLine.Comment
                else
        ...
        if Cust.Get("Sales Invoice Header"."Sell-to Customer No.") then begin
            if (Cust."State Code" <> '') and RecState.Get(Cust."State Code") then
                ExcelBuffer.AddColumn(RecState.Description, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text)
            else
                ExcelBuffer.AddColumn('', FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
        end else
        ...
        //EINV CancelDt
        if "Sales Invoice Header"."E-Inv. Cancelled Date" <> 0DT then
        ...
        //Sales Order Date
        if "Sales Invoice Header"."Order Date" <> 0D then
        ...
        if "Sales Invoice Header"."Sell-to Phone No." <> '' then
            ExcelBuffer.AddColumn("Sales Invoice Header"."Sell-to Phone No.", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
        ...
        //Payment Term Description - Custom Fields
        if "Sales Invoice Header"."Payment Term Details" <> '' then
            ExcelBuffer.AddColumn(CopyStr("Sales Invoice Header"."Payment Term Details", 1, 250), FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text)
        else
            ExcelBuffer.AddColumn('', FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);

        //HSN Code
        if ItemMaster.Get("Sales Invoice Line"."No.") then
            ExcelBuffer.AddColumn(ItemMaster."HSN/SAC Code", false, '', false, false, false, '', ExcelBuffer."Cell Type"::Number)
        else
            ExcelBuffer.AddColumn('', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        Clear(CreditMemoNo);
        Clear(TotalInvGSTAmount);
        Clear(TotalCrGSTAmount);
        SalesCrMemoHeader.Reset();
        SalesCrMemoHeader.SetRange("Applies-to Doc. No.", "Sales Invoice Header"."No.");
        if SalesCrMemoHeader.FindFirst() then begin
            CreditMemoNo := SalesCrMemoHeader."No.";

            // Invoice Total Including GST
            TotalInvGSTAmount := "Sales Invoice Header"."Amount Including VAT";

            // Credit Memo Total Including GST
            TotalCrGSTAmount := SalesCrMemoHeader."Amount Including VAT";

            // Status Logic
            if Round(TotalInvGSTAmount, 1) = Round(TotalCrGSTAmount, 1) then
                InvoiceCancelStatus := 'Fully Cancelled'
            else
                InvoiceCancelStatus := 'Partially Cancelled';

        end else
            InvoiceCancelStatus := 'NA';

        //Invoice cancel status
        ExcelBuffer.AddColumn(InvoiceCancelStatus, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);

        //Credit Memo Number.
        ExcelBuffer.AddColumn(CreditMemoNo, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Number);

        //Invoice Amt. (Total Incl. VAT (INR)
        ExcelBuffer.AddColumn(TotalInvGSTAmount, false, '', false, false, false, '#,##0.00', ExcelBuffer."Cell Type"::Number);
    ...
    begin
        ExcelBuffer.CreateNewBook('Invoice Register Summary');
        ExcelBuffer.WriteSheet('Invoice Register Summary', '_', UserId);
        ExcelBuffer.CloseBook();
        ExcelBuffer.SetFriendlyFilename('Invoice Register Summary' + '_' + UserId);
    ...
    var
        ...
        RegionalGroupName: Text;
        MarginValue: Decimal;

        SalesCrMemoHeader: Record "Sales Cr.Memo Header";
        CreditMemoNo: Code[20];
        InvoiceCancelStatus: Text;
        SalesCrMemoLine: Record "Sales Cr.Memo Line";
        CrIGSTAmt: Decimal;
        CrCGSTAmt: Decimal;
        CrSGSTAmt: Decimal;
        TotalCrGSTAmount: Decimal;
        TotalInvGSTAmount: Decimal;
        CrLineAmount: Decimal;
