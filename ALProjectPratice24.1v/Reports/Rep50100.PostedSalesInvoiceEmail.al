report 50100 "Posted Sales Invoice Email"
{
    ApplicationArea = all;
    Caption = 'Posted Sales Invoice Email';
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(EmailSetup; EmailSetup)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Send Report by Email';
                        // Enabled = LogInteractionEnable;
                        ToolTip = 'Specifies that wheather to send this report through the email address.';
                    }
                    field(NoOfPrints; NoOfPrints)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Number of Prints';
                        // Enabled = LogInteractionEnable;
                        ToolTip = 'Specifies that how many compies of document is print or sent.';
                    }
                }
            }
        }
    ...
    trigger OnPreReport()
    var
        PostedSalesInvoiceEmail: report "Posted Sales Invoice Email";
    ...
    begin
        ...
        if EmailSetup then begin
            Clear(PostedSalesInvoiceEmail);
            Sheader.CopyFilters(Header);
            PostedSalesInvoiceEmail.SendReportByEmail(Sheader, EmailSetup, NoOfPrints);
            // SentEmail('balaji.k@shaligraminfotech.com', 'guru.v@shaligraminfotech.com', 'Test Header', 'Test Body', '', '');
            Message('Email is sent');
            CurrReport.Quit();
        end;
    end;

    procedure SendReportByEmail(var Sh: Record "Sales Invoice Header"; SenEmail: Boolean; no: Integer)
    var
        PostedSalesInvoiceEmail: Report "Posted Sales Invoice Email";
        recRef: RecordRef;
        ShParam: Record "Sales Invoice Header";
    ...
    begin
        ShParam.Reset();
        if SenEmail then begin
            if Sh.FindFirst() then begin
                ShParam.Get(Sh."No.");
                recRef.GetTable(ShParam);
                tmpBlob.CreateOutStream(OutStr);
                // if PostedSalesInvoiceEmail.SaveAs('', format::Pdf, OutStr, recRef) then begin
                if Report.SaveAs(Report::"Posted Sales Invoice Email", '', format::Pdf, OutStr, recRef) then begin
                    tmpBlob.CreateInStream(InStr);
                    txtB64 := cnv64.ToBase64(InStr, true);
                    emailMsg.Create('guru.v@shaligraminfotech.com', 'Invoice', 'Please find attached the invoice.');
                    for i := 1 to no do begin
                        ConvertI := format(i);
                        FileName := 'Invoice' + ConvertI + '.Pdf';
    ...
    // procedure BreakReport(BReport: Boolean)
    // begin
    //     BreakR := BReport;
    // end;
    // procedure SentEmail(FromAddress: Text[250]; SendTo: Text[250]; subject: text[250]; body: Text[250]; AttachmentFilePath: Text[250]; AttachmentName: Text[250])
    // var
    //     email: Codeunit Email;
    //     emailMsg: Codeunit "Email Message";
    // begin
